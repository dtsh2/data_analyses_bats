...
Data are best stored as **.csv** files for our purposes, but R can handle many different source file types ad these may be more appropriate, depending on what you are doing.
...
For example, here's an example R script that imports a .csv file named *example_data.csv* from a folder named *data_folder* and converts it into a dataframe with headers:
...
**Note that this will not work without editing.** Also, we are using the **#** symbol here to *comment* out the text so that it does not run. If we delete the *#* symbol and run it it will produce an error message.
...
The data we imported came from a paper, *SARS-CoV-2 integral membrane proteins shape the serological responses of COVID-19 patients* by Martin et al. You can find the paper here: https://doi.org/10.1016/j.isci.2021.103185 and the data here:
https://data.mendeley.com/datasets/69tvkst9ct/1 

It's the type of data we will be collecting during this project, except that they are from people and not bats.
...
Here's an example of how to change the data type of the *Sex* column to a *factor*:
...
Note that you get a **warning**, but this is fine. This is warning us that there are **NA**s. These are fine and telling us that we have missing data.
...
Personally, I think plotting data is extremely helpful. It tells us so much and helps make sure that the data are as we expect, plus it tells us what we might see in the analyses, even before we have performed them. Having plotted your data can also help you identify if you have errors in your analyses, because you might get a result that is unexpected. This might be a real result or an error.

*Note that in this section, like others, we are not necessarily always going to use the most useful plot for the data, because this is a workshop.*
...
And what do the titres look like by age and gender (here classified as *Sex*):
...
example_data %>%
  ...
  ggplot( aes(x=myaxis, y=IgG_S, fill=Sex)) +
    ...
    ggtitle("A 'Violin' wrapping a boxplot, coloured by 'Sex' ") +
    xlab("") +
  ylab("IgG S")
```

We will now break this down, but you do not need to learn this, this is just to help see how code works:

* *example_data %>%* is the data set and uses the *pipe* symbol (%>%) to say what to do with it and tell R that there is more to come.

* *left_join(sample_size) %>%* the **left_join()** function in R is used to combine two data frames based on a common column, keeping all the rows from the left data frame and adding matching rows from the right data frame.
...
And last for this session, let's plot the different antibodies types (*IgG*, *IgM*, *IgA*) against the spike protein (*S*) through time. 

First, for *ggplot* we should convert the **wide format** data to **long format** data before plotting it, because to plot these data in ggplot, we need to **reshape** the data so that each row contains a single observation of the variable at a given time point. To do this, we can use the **gather()** function from the **tidyr** package:
...
But what we might be interested in is how titres vary with time, so let's plot that:
