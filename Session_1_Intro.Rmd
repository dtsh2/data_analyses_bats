...
Do not worry about the **ggplot** text for now, we will have a session on this, because *ggplot* uses a slightly different syntax. We will cover that syntax but one thing we won't cover here is *shortcuts*, but you will learn them as you code more.
...
This is also a good place to note two other important R syntax points: R treats capital (or *upper case*) letters differently to *lower case*, so A is not the same a, meaning if you have a vector of characters or factors, Apple and apple with are different. Also, you can also assign things using the = sign instead of the *assignment operator* **<-**. So lets do one exercise to test this:
