...
R is widely used by statisticians, data analysts, and researchers for data analyses and visualization.

#### R

**R** is a programming language used for statistical computing and graphics. It was first developed by Ross Ihaka and Robert Gentleman at the University of Auckland, New Zealand in 1993, which I mention as I live in New Zealand.
...
*n2* includes an **NA**, which means* *Not Available*, and so the function mean() cannot calculate the mean when a number in does not exist. We can easily resolve that here because mean() has a *logical* condition that we will learn more about later, but here it is **na.rm = FALSE**. We include that and get a different answer:
...
Now, R comes with some built in data sets as well as functions. We can use another function, **summary()** to see what is in one data set called *cars*:
...
Do not worry about the **ggplot()** text for now, we will have a session on this, because *ggplot()* uses a slightly different **syntax**.
...
R packages allow users to perform a wide range of specialized tasks in R, from data visualization to statistical analysis to machine learning. Some popular R packages include **ggplot2** for creating visualizations, **dplyr** and **tidyr** for data cleaning and manipulation, and **caret** for machine learning.
...
R treats capital (or *upper case*) letters differently to *lower case*, so **A is not the same a**. This means that if you have a vector of characters or factors with *Apple* and *apple*, these are different. Similarly if you import a dataset that has a *space* before or after a word, it is different. So, ' Cow' is not the same as 'Cow' is not the same as ' cow' or 'cow' to R. 

##### Assignment operators

Also, you can also assign things using the **=** sign instead of the *assignment operator* **<-**. So lets do one exercise to test this:
...
We can plot time and date data too. Here's an example of some code, simulating data from a different time period just to show you another data series and here we will plot it with **ggplot()**:
