...
The vector we made called *n2* includes an **NA**, which means *Not Available*, and so the function **mean()** cannot calculate the mean when a number in does not exist. We can easily resolve that here because mean() has a **logical** condition that we will learn more about later, but here it is **na.rm = FALSE**. We include that and get a different answer:
...
In this script, a **factor** variable *var1* is defined with 9 values representing 3 different **levels**: "A", "B", and "C". A **numeric** variable *var2* is also defined with 6 values.
