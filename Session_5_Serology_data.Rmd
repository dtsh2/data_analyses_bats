...
Not that these data are **skewed data**. 
...
#### Determining differences; cut offs, ROC, and test characteristics
...
Now let's assume that these are from the same sample but the *IgA_S* with the lower titres are **naive** animals that have never has an infection (so susceptible in the modelling), and the *IgG_S* are the **immune** or if not immune **recovered** animals (recovered) that are truly **seropositive**, but we want to find the difference. 

You can see that some of the *"negative"* animals have titres that are higher than the *"positive"* animals. This is not uncommon. Many things determine this, such as previous infections or waning antibody titres. These lead to **false positive** and **false negative** results.
...
We can quickly look at this using some simple maths and then a piece of code. 
...
This provides a huge amount of information on the tests. 
...
Interpreting the AUC value is straightforward. The higher the AUC, the better the model's ability to classify positive and negative samples accurately. An AUC of 0.5 indicates a model that performs no better than chance, while values above 0.5 indicate better-than-chance performance. An AUC of 1 suggests a model with flawless discrimination.
...
Notice two things:
* first, that this has a higher AUC so if this were a real test and we were comparing it to the previous analysis, this would be *better*.
* second, note that there are **two** *threshold* possibilities! Let's see those:
...
In this case, the mixed model would involve fitting two *Gaussian distributions* or *normal* distributions to the data, with each distribution corresponding to one of the bimodal peaks. The means and standard deviations of each distribution would be modeled as fixed effects, while the within-subject variability would be modeled as a random effect.

The mixed model would also include a binary variable indicating which of the two distributions each observation belongs to. The cut off point between the two distributions would be estimated as the point where the probability of belonging to each distribution is equal.

The mixed model would be fit using **maximum likelihood** estimation, and model selection methods such as **Akaike's Information Criterion** (AIC) or **Bayesian** methods and **Bayesian Information Criterion** (BIC) could be used to determine the best model. We will cover these better in **Session 7**
