...
Note what different information they give you. 
...
Note the output will show the average *mpg*, *cyl*, and *hp* as separate columns named *avg_mpg*, *avg_cyl*, and *avg_hp*, respectively.
...
Note that without using the **set.seed()** function, we will all have different plots because **rpois()** and the replated functions usings a random number generator. You can take a look by typing **?Distributions** in the console.
...
Now let's increase the simulated data. This can be done simply, such as by replicating it using the **replicate()** function any number of times. Here we will replicate the above data three times using this code:
...
The points are now visible because *geom_jitter()* adds some **noise** (random variation) to the points. The *amount of jitter* can be controlled by using the **height** and **width** arguments. For example, using smaller values \<1 makes the points *tighter*, like this:
