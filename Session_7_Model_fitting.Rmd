...
**Session 7** contains an introduction to fitting models to data.
...
Some mechanistic mathematical models are run without fitting the models to data, but often an informative way to understand a system is to fit models to the data, potentially in an iterative way, where models are developed, data collected based on the model parameters, the model fit to the data, which generates news questions, etc. in an iterative fashion until the system is well understood.
...
The methods used, but mostly not thought about is a type of **Generalized Least Squares Estimation** (or **GLS**) method called **Ordinary Least Squares** (or **OLS**). OLS makes two crucial assumptions:
...
However, interestingly (or not!) really maximum likelihood is a special case of a Bayesian model and GLS a special case of maximum likelihood, like OLS is a special case of GLS. So, they are all related *but* they all make **different assumptions**.
...
**bbmle** is also able to fit data to **dynamic models** where the value of the **state variables** like the **susceptible, infected and recovered** in **SIR models**, changes through time. When this happens, the distributions are not likely to be normal.
...
Here's how we can break that down. First, the likelihood estimation can be considered to be four parts:
...
The function **yhat()** represents the deterministic part of the model and **rpois()** adds stochasticity (noise, or randomness).
...
We pass the optimization function logit-transformed values which the function then back-transforms using **plogis()**. Similarly, for the population size and number of initial infections, we pass log-transformed values which the function back-transforms. See if you can find these terms in this code.
...
Note that this does not do at estimating standard errors - you can see the **NaN** which means that they have not been estimated.
...
Bayes' theorem quantifies the **posterior probability** of an event given **prior knowledge** and **observed data**. The prior represents our initial beliefs, while the **likelihood** function describes how the data inform us about the underlying parameters of interest.
...
Not that Bayesian analyses are closely linked to the concept of **likelihood**, a fundamental component of statistical inference. The likelihood function, often denoted as **L(θ|X)**, describes the *probability of observing the data X given a specific set of parameters θ*. It quantifies the plausibility of different parameter values, given the observed data.
...
Here we will introduce Bayesian analyses through using another piece of software and R package, **JAGS** (Just Another Gibbs Sampler).
...
We then create and run the model it. We will run it for 4 chains (runs) and allow our model to be run in the adaptive phase.  We won't cover that here.
...
The *hairy caterpillar* looking plot and *normal* distribution is a good indication that there has been good mixing and convergence of the model.

Let's look at these in other ways using different functions. We can save the results as a new data set:
...
Now you can see that these hostograms show the same results for the parameter estimates from the summaries of the data using the **summary()** function above.
