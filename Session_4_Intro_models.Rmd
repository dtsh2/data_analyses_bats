...
Models act as simplified versions of reality, enabling us to navigate and interact with the world more effectively. They come in various forms, ranging from physical replicas to experimental systems to mathematical equations and computer simulations. By focusing on key variables and relationships, models enable us to analyze and begin to understand systems that might otherwise overwhelm us with their complexity.
...
Let's import our newly created and saved data set, **example_data.csv** and begin:
...
The **method = "lm"** argument is used to fit a linear model to the data and *add the regression line*. We have a **confidence interval** in *grey* around the line, but could remove that using the *se = FALSE* argument. The **ggtitle()**, **xlab()**, and **ylab()** functions are used to add a title and axis labels to the plot.
...
The results of this would suggest that in our data set age does not significantly affect the IgG anti-spike antibody titre, but remember there are a lot going on here and we need to consider other factors.
...
In the simplest SIR model we assume there are no births or deaths. 
...
Once immune, they remain so for life (i.e. they do not leave the recovered class). 
...
### Session 2b - Introduction to running different models
...
This code uses the **deSolve** package in R to simulate the **SIR model** using the **Runge-Kutta** method. 
